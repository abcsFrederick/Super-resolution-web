...
def _inference(args, model, IMAGE_SIZE, BATCH_SIZE, file_name, PREDICTION_PATH, num_classes, kernel):
    ...
    print('Name of the file to be inferenced: ', file_name)
    ...
    output_path = PREDICTION_PATH + dir_string + '_' + basename_string + '.h5'
    ...
    print(f'image org shape: {image_org.shape}, image type: {image_org.dtype}')
    print('')
    ...
    # imwrite(PREDICTION_PATH + dir_string + '_' + basename_string + '_pr.tif', prob_map_valid)
    ...
    num_subifds = 8
    with tifffile.TiffWriter(PREDICTION_PATH + dir_string + '_' + basename_string + '_pr.ome.tif', bigtiff=True) as tif:
...
def inference_WSIs(args, model, image_size, batch_size, file_name, output_path, num_classes):
    ...
    _inference(args, model, image_size, batch_size, file_name, output_path, num_classes, kernel)
...
def cycle_gan_test():
    ...
    inference_WSIs(opt, model.netG_A, opt.crop_size, 16, file_name, opt.results_dir, 1)

if __name__ == '__main__':
    ### Read OME TIF test first
    ometif_path = './Data/CZ_ESR105/3034_G12C_pflox_veh_frame_6_channel_AF555.ome.tif'
    mask_stack_image = tifffile.imread(ometif_path)
    print(f'ome.tif shape: {mask_stack_image.shape}, dtype: {mask_stack_image.dtype}')

    png_path = './Data/CZ_ESR105/3034_G12C_pflox_veh_frame_6_channel_AF555.png'
    png_image = skimage.io.imread(png_path)
    print(f'png shape: {png_image.shape}, dtype: {png_image.dtype}')
    cycle_gan_test()

# python image_from_omero_ometif.py --dataroot ./datasets/low2high --name esr105_cyclegan3_interpol_batch8 --model cycle_gan --gpu_ids 2 --load_size 256 --crop_size 256 --load_iter 0 --file_name './Data/CZ_ESR105/3034_G12C_pflox_veh_frame_6_channel_AF555.ome.tif'